...
#create a list of telegram usernames to authorise them, 0th username is myadmin.
usern_list = []
# Define a few command handlers. These usually take the two arguments bot and
# update. Error handlers also receive the raised TelegramError object in error.
def start(bot, update):
    """Send a message when the command /start is issued."""
    reply = "Welcome \nSend /help command if you are newbe here :)) "
    update.message.reply_text(reply)


def help(bot, update):
    """Send a message when the command /help is issued."""
    myadmin = update.message.from_user.username
    if myadmin == usern_list[0]:
...
def show_user(bot, update):
    myadmin = update.message.from_user.username
    if myadmin == usern_list[0]:
        update.message.reply_text(usern_list)
    else:
        update.message.reply_text("You are not Authorized.")
...
def main():
    ...
    mydispatcher.add_handler(CommandHandler("get", get))
    mydispatcher.add_handler(CommandHandler("ls", ls))
    mydispatcher.add_handler(CommandHandler("put", put))
    mydispatcher.add_handler(CommandHandler("mkdir", mkdir))

    #admin 
    mydispatcher.add_handler(CommandHandler("adduser", add_user))
    mydispatcher.add_handler(CommandHandler("showuser", show_user))
    mydispatcher.add_handler(CommandHandler("removeUser", remove_user))
    mydispatcher.add_handler(CommandHandler("remove", remove))
    mydispatcher.add_handler(CommandHandler("rmdir", rmdir))

    # on noncommand 
    mydispatcher.add_handler(MessageHandler(Filters.document, echo))

    # log all errors
    mydispatcher.add_error_handler(error)
